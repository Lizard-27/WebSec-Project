  ...
  <div class="col-md-4">
    <div class="card h-100">
      ...
      <div class="card-body d-flex flex-column">
        ...
        <!-- Add to Cart Form (inside loop) -->


        <!-- Buy Button -->
        @auth
          @if($product->quantity > 0 && auth()->user()->credit >= $product->price)
            <a href="{{ route('products.show', $product->id) }}" class="btn btn-primary mt-2">Buy</a>
          @elseif($product->quantity > 0)
            <button class="btn btn-secondary mt-2" disabled>Insufficient Credit</button>
          @else
            <button class="btn btn-secondary mt-2" disabled>Out of Stock</button>
          @endif
        @else
          <a href="{{ route('login') }}" class="btn btn-warning mt-2">Login to Buy</a>
        @endauth
